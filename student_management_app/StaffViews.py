...
# Add these views to staff_views.py or create a new file if needed

def staff_create_club(request):
    if request.method == "POST":
        ...
        try:
            club = Club(name=club_name, description=description, staff_in_charge=staff)
            club.save()
            ...
            return redirect('staff_create_club')
    else:
        return render(request, 'staff_template/staff_create_club.html')




def staff_manage_clubs(request):
    staff = Staffs.objects.get(admin=request.user.id)
    clubs = Club.objects.filter(staff_in_charge=staff)
    
    context = {
        "clubs": clubs
    }
    return render(request, 'staff_template/staff_manage_clubs.html', context)


def staff_view_club_applications(request, club_id):
    staff = Staffs.objects.get(admin=request.user.id)
    club = Club.objects.get(id=club_id)
    
    # Verify the staff is in charge of this club
    if club.staff_in_charge != staff:
        messages.error(request, "You are not authorized to manage this club!")
        return redirect('staff_manage_clubs')
    
    applications = ClubMembership.objects.filter(club_id=club)
    
    context = {
        "club": club,
        "applications": applications
    }
    return render(request, 'staff_template/staff_view_club_applications.html', context)

def staff_approve_club_application(request, application_id):
    try:
        staff = Staffs.objects.get(admin=request.user.id)
        application = ClubMembership.objects.get(id=application_id)
        
        # Verify the staff is in charge of this club
        if application.club_id.staff_in_charge != staff:
        ...
        return redirect('staff_view_club_applications', application.club_id.id)
...
def staff_reject_club_application(request, application_id):
    try:
        staff = Staffs.objects.get(admin=request.user.id)
        application = ClubMembership.objects.get(id=application_id)
        ...
        if request.method == 'POST':
            ...
            application.feedback = request.POST.get('feedback', 'Application rejected')
            application.save()
